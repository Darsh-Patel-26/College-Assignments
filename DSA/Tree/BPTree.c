...
node *deleteEntry(node *root, node *n, int key, void *pointer)
{
    ...
    if (i < n->num_keys)
    {
        ...
        // If the root is the only leaf and there are no keys left, adjust the root
        if (n == root && n->num_keys == 0)
            root = NULL;
...
// Function to delete a key from a non-leaf (internal) node
node *deleteEntryInternal(node *root, node *n, int key, void *pointer)
{
    int i = 0;
    while (i < n->num_keys && n->keys[i] != key)
        i++;
...
// Function to adjust the root after deletion
...
// Function to delete a key and its associated record
node *deleteKey(node *root, int key)
{
    ...
    root = deleteEntry(root, leaf, key, NULL);
...
int main()
{
    ...
    root = insert(root, 12, 18);
    root = insert(root, 22, 33);
    root = insert(root, 42, 56);
    root = insert(root, 55, 22);
    ...
    root = insert(root, 17, 21);
    printTree(root);
    printf("\n");

    findAndPrint(root, 22, instruction = 'a');
